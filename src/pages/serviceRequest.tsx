...
async function updateServiceRequest(id: string, patch: Partial<ServiceRequest>) {
	await http.put(`/service-requests/${id}`, patch);
}
...
export default function ServiceRequestPage() {
	...
	useEffect(() => {
		(async () => {
			try {
				setLoading(true);
				setRows(await fetchServiceRequests());
				setError(null);
			} catch {
	...
	const filtered = useMemo(() => {
		...
		return rows.filter((d) =>
			[d.id, d.order_id, d.dealer_id, d.status]
				.filter(Boolean)
				.some((v) => String(v).toLowerCase().includes(q))
		);
	}, [rows, search]);

	const handleViewDetails = (d: ServiceRequest) => {
		setSelected(d);
		setDetailOpen(true);
	};

	const handleDelete = async (d: ServiceRequest) => {
		if (window.confirm(`Delete Service Request "${d.id}"?`)) {
			try {
				await http.delete(`/service-requests/${d.id}`);
				setRows((prev) => prev.filter((r) => r.id !== d.id));
			} catch {
				window.alert("Failed to delete Service Request. Please try again.");
			}
		}
	};
	...
	const processRowUpdate = async (newRow: GridRowModel, oldRow: GridRowModel) => {
		...
		if (Object.keys(patch).length) {
			await updateServiceRequest(id, patch);
											...
											<CardContent sx={{ py: 1.25 }}>
												<Box sx={{ display: "flex", alignItems: "center", mb: 1 }}>
													...
													<Box sx={{ minWidth: 0, flex: 1 }}>
														<Typography fontWeight={700} noWrap title={d.id}>
															{d.id}
														</Typography>
